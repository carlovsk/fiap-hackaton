...
describe('VideoService', () => {
  ...
  describe('uploadVideo', () => {
    it('should upload file, save video and publish event', async () => {
      ...
      mockRepository.create.mockResolvedValue({ id: videoId, userId, filename: file.originalname, status: 'PENDING', createdAt: new Date(), downloadKey: null });

      const result = await service.uploadVideo(userId, file);

      expect(uuid).toHaveBeenCalled();
      expect(mockUploadService.uploadFile).toHaveBeenCalledWith({ key, file });
      expect(mockRepository.create).toHaveBeenCalledWith({ id: videoId, userId, filename: file.originalname, status: 'PENDING' });
      expect(mockPublisher.connect).toHaveBeenCalled();
      expect(mockPublisher.publish).toHaveBeenCalledWith('video.uploaded', { videoId, userId, filename: file.originalname, key });
      expect(result).toEqual({ id: videoId, userId, filename: file.originalname, status: 'PENDING', createdAt: expect.any(Date), downloadKey: null });
    });
  ...
  describe('getVideoDownload', () => {
    ...
    it('should return null when user mismatch', async () => {
      const video = { id: 'id', userId: 'other', filename: 'a', status: 'COMPLETED', downloadKey: 'key', createdAt: new Date() };
    ...
    it('should return null when video not ready', async () => {
      const video = { id: 'id', userId: 'user', filename: 'a', status: 'PENDING', downloadKey: null, createdAt: new Date() };
      mockRepository.findById.mockResolvedValue(video);
      const result = await service.getVideoDownload('id', 'user');
      expect(result).toBeNull();
    });
    ...
    it('should return null when file download fails', async () => {
      const video = { id: 'id', userId: 'user', filename: 'a', status: 'COMPLETED', downloadKey: 'key', createdAt: new Date() };
      mockRepository.findById.mockResolvedValue(video);
  ...
  describe('markVideoAsProcessed', () => {
    it('should update video status and downloadKey', async () => {
      const payload = { videoId: faker.string.uuid(), userId: faker.string.uuid(), status: 'COMPLETED' as const, downloadKey: faker.string.uuid() };
      mockRepository.updateById.mockResolvedValue(undefined);

      await service.markVideoAsProcessed(payload);

      expect(mockRepository.updateById).toHaveBeenCalledWith(payload.videoId, { status: payload.status, downloadKey: payload.downloadKey });
    });
  });
});

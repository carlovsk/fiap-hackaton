...
export class SQSMessageConsumer implements IMessageConsumer {
  ...
  async startListening(): Promise<void> {
    ...
    if (!env.SQS_PROCESSED_QUEUE_URL) {
      throw new Error('SQS_PROCESSED_QUEUE_URL not configured');
    }

    this.isListening = true;
    this.logger.info(`Video service is listening for messages on SQS queue: ${env.SQS_PROCESSED_QUEUE_URL}`);
  ...
  private async pollMessages(): Promise<void> {
    if (!this.client || !env.SQS_PROCESSED_QUEUE_URL) {
      return;
    }
